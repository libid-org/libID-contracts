...
///      here is `resolveHandle` and nothing else, so whoever can decide what
///      that answers can take what is held. Two keys can: the naming owner, through
...
///      owners"; they are the keys that already decide which proofs mint names
///      at all, so this adds no new party — only a new thing those keys can
///      reach.
contract HandleEscrow is Initializable, UUPSUpgradeable, Ownable2StepUpgradeable, ReentrancyGuardTransientUpgradeable {
    ...
    /// @param handle     The handle, as written. Case, surrounding spaces and
    ///                   one leading at-sign do not matter: they are the same
    ///                   handle spelled another way.
